function NR_Burgers_KS_RKCB3c_PS
...
for k=1:Tmax/dt
  for rk=1:4 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ALL 3 RK SUBSTEPS %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if (rk==1)                                                       % 2 FFTs per RK step
        ...
        y=uhat+(Aim(rk,rk-1)-b(rk-1))*z + (Aex(rk,rk-1)-b(rk-1))*y;
    end
      z=(Aop.*y)./(1-Aim(rk,rk)*Aop);
      r=NR_RFFTinv(y+Aim(rk,rk)*z,N); r=-0.5*r.*r; y=i*kx.*NR_RFFT(r,N);                % Leading-order cost:
      uhat=uhat+b(rk)*z+b(rk)*y;
  end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END OF RK LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
